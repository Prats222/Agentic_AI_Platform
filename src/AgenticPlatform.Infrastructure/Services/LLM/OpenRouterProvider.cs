...
public sealed class OpenRouterProvider : ILLMProvider
{
    ...
    public async Task<LLMChatResponse> ChatAsync(LLMChatRequest request, CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new InvalidOperationException("OpenRouter requires an API key.");
        }

        var endpoint = BuildEndpoint(request.BaseUrl, "https://openrouter.ai/api/v1", "chat/completions");
        ...
        {
            throw new LLMProviderException("OpenRouter", response.StatusCode, raw);
        }
